function clearAll() {
    ...
    if (con.contains(document.querySelector('#meme')))
        document.querySelector('#meme').remove()
    if (con.contains(document.querySelector('#joke')))
        document.querySelector('#joke').remove()
    if (con.contains(document.querySelector('#qoute')))
        document.querySelector('#qoute').remove()
    if (con.contains(document.querySelector('#riddle')))
        document.querySelector('#riddle').remove()
    if (con.contains(document.querySelector('#answer')))
...
function showMeme() {
    ...
    el.src = randomMemeUrl
    el.setAttribute('id', 'meme')
    const con = document.querySelector('.content')
    const secEl = con.children.item(1)
    con.insertBefore(el, secEl)
...
function showJoke() {
    ...
    el.setAttribute('id', 'joke')
    el.innerHTML = randomJokeText
    const secEl = document.querySelector('[id="gq"]')
    const con = document.querySelector('.content')
    con.insertBefore(el, secEl)
}
...
function showQuote() {
    ...
    const el = document.createElement('p')
    el.setAttribute('id', 'qoute')
    el.innerHTML = `${randomQuote.quote}
${randomQuote.author}
  `
    const con = document.querySelector('.content')
    const secEl = document.querySelector('[id="gr"]')
    con.insertBefore(el, secEl)
}
...
function showRiddle() {
    ...
    const el = document.createElement('p')
    el.setAttribute('id', 'riddle')
    el.innerHTML = `${randomRiddle.question}`

    const ela = document.createElement('p')
    ela.setAttribute('id', 'answer')
    ela.innerHTML = `${randomRiddle.answer}`
...
function revealAnswers() {
    if (document.querySelector('.content').contains(document.querySelector('#riddle'))) {
        document.getElementById('answer').style.visibility = 'visible'
    } else if (!document.querySelector('.content').contains(document.querySelector('#answer'))) {
        const ela = document.createElement('p')
        ela.setAttribute('id', 'answer')
        ela.innerHTML = "you don't have a riddle 3mmo"
        document.querySelector('.content').appendChild(ela)
    }
}
